...
public class DrawingModule
{
    ...
    public static void DrawGraphOnCircle(Graph graph)
    {
        ...
        for (var i = 1; i <= graph.Rows; i++)
        {
            ...
            gfx.DrawString((i + 1).ToString(), new Font("Arial", 10), Brushes.Black, textX + circleRadius / 3,
                textY + circleRadius / 3);
            ...
            foreach (var neighbour in n)
            {
                ...
                gfx.DrawLine(new Pen(Brushes.Black, 2), circleX + circleRadius / 2, circleY + circleRadius / 2,
                    x1 + circleRadius / 2, y1 + circleRadius / 2);
